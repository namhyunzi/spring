 ...
 *  			- 멤버변수에 직접 값을 대입하기 
 *  			- 생성자 메소드로의 초기화작업으로 값을 대입하기 
 *  	- 스프링에서 의존성 주입이 필요한 멤버변수에 final 키워드를 지정하고,
 *  	 	@RequiredArgsConstructor를 클래스에 추가하면
 *  	 	스프링 컨테이너가 자동으로 의존성 주입한다. 
 */

@Service
@RequiredArgsConstructor
public class UserService {
	// 회원가입시 비밀번호 암호화를 위해서 PasswordEncoder 구현체를 주입받는다. 
	 ...
	 * 			6. 신규 회원은 ROLE_USER 권한을 부여하기위해서 UserRole 객체를 생성해서 사용자 번호와 "ROLE_USER"객체를 대입한다.
