...
<script type="text/javascript">
	...
	xhr.onreadystatechange = function() {				// 2. xhr객체의 readyState 값이 변경될 때마다 실행되는 이벤트 핸들러 함수를 등록한다. 
		if (xhr.readyState == 4 && xhr.status == 200) {	// 5. readyState=4:응답이 왔다. status=200:성공
			let text = xhr.responseText;				// 6. xhr의 responseText 프로퍼티에서 텍스트 응답데이터를 조회한다. 
			console.log(text);
	...
	xhr.open("GET", "/admin/users/" + id);				// 3. xhr 객체를 초기화한다. 요청방식, 요청URL을 지정한다.
