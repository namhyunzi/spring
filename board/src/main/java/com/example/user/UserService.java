 ...
 * 		- UserRepository 인터페이스 타입이다. 
 * 		- userRepository 프로퍼티에는 UserRepository 인터페이스의 구현객체가 의존성 주입된다.
 * 		- UserRepository 인터페이스의 구현객체는 Spring Data JPA 구현체를 이용해서 구현하고, 스프링 컨테이너의 빈으로 등록한다. 
 */
@Service
...
/*	아래와 동일한 메소드(다른 방식으로 작성한)
	public User getUser(Long id) {
		Optional<User> optional =  userRepository.findById(id);
		if (optional.isEmpty()) {
			throw new RuntimeException("사용자 정보가 존재하지 않습니다");
		}
		return optional.get();	// get하면 user가 반환된다.
	}
*/
	/**
	 * 사용자의 기본키값을 전달받아서 사용자 정보를 반환한다. 
	 * @param id 사용자 기본키 값
