 ...
 * 			@SpringBoot@Configuration - 사용자가 추가적으로 빈이나 설정 클래스를 등록 가능하게 한다.
 ...
 *				- 사전에 미리 정의한 라이브러리 의존성을 컨테이너의 빈으로 자동 등록되도록 한다.
 *				  (스프링부트는 자동으로 라이브러리 의존성을 기반으로 사전에 미리 정의된 객체를 스프링 컨테이너의 빈으로 사용등록하고, 의존성 주입까지 완료한다.)
 *
 ...
 *    			- @Controller, @RestController, @ControllerAdivice @RestControllerAdvice
 *    			- @Service, @Repository
 *    			  @Component
 *                @Configuration
 * 			
